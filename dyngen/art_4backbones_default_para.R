...
for(data_num in 1:4){
  for(net_id in 1:20){
    ...
    if(T){
      ...
      kinetics_change1 <- function() {
        ...
        sampler_nontfs <-
          function(feature_info, feature_network, cache_dir = NULL, verbose = FALSE) {
            feature_info %>% mutate(
              transcription_rate = transcription_rate %|% runif(n(), alpha_min, alpha_max),
              translation_rate = translation_rate %|% runif(n(), 100, 150),
              mrna_halflife = mrna_halflife %|% runif(n(), 1, 10),
              protein_halflife = protein_halflife %|% runif(n(), 5, 10),
              independence = independence %|% 1,
    ...
    save(model,file = paste(dir,'dataset',data_num,'_net',net_id,'.RData',sep = ''))
    print(paste('dataset',data_num,'_net',net_id,sep = ''))
  }
}
  ...
  for(net_id in 1:20){
    for(rep_id in 0:0){
      ...
      if(rep_id!=0){  # if rep_id ==0 -> use all cells to infer network
        ...
        sub.t = sub.t[which(sub.t>(rep_id-1)*length(t)/num_simu & sub.t<=(rep_id)*length(t)/num_simu)]
      }
      ...
      for(seed_id in 1:1){
        ...
        print(paste('dataset',data_num,'_net',net_id,"_rep",rep_id,"_seed",seed_id," inex done!",sep = ''))
        save(weightMatrix, file = paste(dir,'Network_inference/genie3_output_inex_',data_num,'_net',net_id,"_",
        ...
        print(paste('dataset',data_num,'_net',net_id,"_rep",rep_id,"_seed",seed_id," exon done!",sep = ''))
        save(weightMatrix, file = paste(dir,'Network_inference/genie3_output_exon_',data_num,'_net',net_id,"_",
                                        rep_id,"_",seed_id,'.RData',sep = ""))
      }
  ...
  for(net_id in 1:20){
    for(rep_id in 0:0){
      for(seed_id in 1:1){
        ...
        # Evaluation
        print(paste(data_num,net_id,rep_id,seed_id,"Start!!!",sep = " "))
        mat = exon.mat
        if(T){
          A1 = mat
          ...
          # calculate top precision
          TP = 0
          thr = 0.1
          id = which(recall<thr)[1]
          for(i in id:interval){
            TP = TP+ (recall[i] - recall[i+1])*(precision[i]+precision[i+1])/2
          }
          TP = TP/recall[id]
          
          
          # df:  data_num,net_id,num_genes,rep_id, seed_id,input, EPR, AUROC, AUPR, EP, random precision,TP
          df.tmp = c(data_num,net_id,num_genes,rep_id,seed_id,"exon", EPR, AUROC, AUPR, EP, random_precision,TP)
          df = rbind(df,df.tmp)
          
          print(paste(data_num,net_id,rep_id,seed_id,"exon","Done!!!",sep = " "))
        }
        ...
        if(T){
          ...
          # df:  data_num,net_id,num_genes,rep_id, seed_id,input, EPR, AUROC, AUPR, EP, random precision,TP
          df.tmp = c(data_num,net_id,num_genes,rep_id,seed_id,"inex", EPR, AUROC, AUPR, EP, random_precision,TP)
          df = rbind(df,df.tmp)
          
          print(paste(data_num,net_id,rep_id,seed_id,"inex","Done!!!",sep = " "))
...
colnames(df) = c("data_num","net_id","num_genes","rep_id", "seed_id","input", "EPR", "AUROC", "AUPR", "EP",
                 "random precision","TP")
rownames(df) = NULL
df = data.frame(data_num = as.numeric(df[,1]),net_id = as.numeric(df[,2]),num_genes = as.numeric(df[,3]),
                rep_id = as.numeric(df[,4]),seed_id = as.numeric(df[,5]),input = as.character(df[,6]),EPR = as.numeric(df[,7]),
                AUROC = as.numeric(df[,8]),AUPR = as.numeric(df[,9]), EP = as.numeric(df[,10]),
                random_precision = as.numeric(df[,11]),TP = as.numeric(df[,12]))

save(df,file = paste(dir,"evaluation/variation_of_config_all_backbones.RData",sep = ""))
